...
public class QuickSortDbl {
    ...
    // This method generates a sorted sequence of doubles from 0 to n-1
    public static double[] generateSortedSequence(int n) {
        double[] arr = new double[n];
        for (int i = 0; i < n; i++) {
            arr[i] = i; // Generate sorted sequence
        }
        return arr;
    }
        ...
        for (int size : sizes) {
            ...
            try (BufferedReader br = new BufferedReader(new FileReader("doubles.txt"))) {
                ...
                for (int run = 0; run < numRuns; run++) {
                    double[] sortedData = generateSortedSequence(size);
